...
try:
    ...
    with tab1:
        ...
        # CUERPO - 7 INDICADORES (Anchos corregidos)
    ...
    with tab2:
        ...
        t1, t2 = st.tabs(["🎂 Cumpleaños de Vida", "🎖️ Aniversarios Laborales"])

        with t1:
            df_v = df_cump[df_cump['DT_NAC'].dt.month == sel_mes].copy()
            if not df_v.empty:
                df_v['DIA'] = df_v['DT_NAC'].dt.day
                rows = [df_v.iloc[i:i+3] for i in range(0, len(df_v), 3)]
                for row_df in rows:
                    cols = st.columns(3)
        ...
        with t2:
            ...
            if not df_l.empty:
                ...
                rows_l = [df_l.iloc[i:i+3] for i in range(0, len(df_l), 3)]
                for row_df in rows_l:
                    cols = st.columns(3)
                    for i, (_, r) in enumerate(row_df.iterrows()):
                        with cols[i]:
                            with st.container(border=True):
            ...
            else: st.info("No hay aniversarios este mes.")

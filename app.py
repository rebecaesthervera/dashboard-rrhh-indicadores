    ...
    with col_centro:
        ...
        with c_sup1:
            with st.container(border=True):
                ...
                if 'GÉNERO' in df_fil.columns and not df_fil['GÉNERO'].isnull().all():
                    ...
                    # Forzamos los textos por dentro
                    fig_g.update_traces(textposition='inside', textinfo='label+percent', insidetextorientation='horizontal')
                    fig_g.update_layout(margin=dict(t=10, b=10, l=10, r=10), height=200, showlegend=False)
                    # theme=None evita que Streamlit borre nuestros textos
        ...
        with c_sup2:
            with st.container(border=True):
                ...
                if 'CATEGORÍA' in df_fil.columns and not df_fil['CATEGORÍA'].isnull().all():
                    ...
                    fig_c.update_traces(textposition='inside', textinfo='label+percent', insidetextorientation='horizontal')
                    fig_c.update_layout(margin=dict(t=10, b=10, l=10, r=10), height=200, showlegend=False)
        ...
        with st.container(border=True):
            ...
            if 'PUESTO' in df_fil.columns and not df_fil['PUESTO'].isnull().all():
                # Forzamos el nombre de las columnas al contar para no depender de la versión de Pandas
                ...
                # cliponaxis=False asegura que los números no se borren al tocar el borde del gráfico

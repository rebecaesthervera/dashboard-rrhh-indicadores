...
try:
    ...
    with tab1:
        if not df_main.empty:
            ...
            # --- TARJETA PRINCIPAL DE MÉTRICAS UNIFICADAS ---
            ...
            if 'EDAD' in df_fil.columns:
                ...
                if not df_edad_valida.empty:
                    ...
                    with sub_c1:
                        st.markdown(f"<p style='margin:0; font-size:12px; color:#64748B; font-weight:600;'>EDAD PROMEDIO</p><p style='margin:0; font-size:16px; font-weight:700; color:#1E3A8A;'>{promedio_edad:.1f} años</p>", unsafe_allow_html=True)
                    with sub_c2:
                        st.markdown(f"<p style='margin:0; font-size:12px; color:#64748B; font-weight:600;'>SEGMENTO CRÍTICO (+46 AÑOS)</p><p style='margin:0; font-size:16px; font-weight:700; color:#1E3A8A;'>{cant_adultos} ({porc_adultos:.1f}%)</p>", unsafe_allow_html=True)
                    with sub_c3:
                        st.markdown(f"<p style='margin:0; font-size:12px; color:#64748B; font-weight:600;'>ÁREA CON MAYOR DENSIDAD SENIOR</p><p style='margin:0; font-size:16px; font-weight:700; color:#1E3A8A;'>{area_senior}</p>", unsafe_allow_html=True)

...
try:
    ...
    with tab1:
        ...
        with col_izq:
            ...
            with st.container(border=True):
                st.dataframe(df_fil[['APELLIDO Y NOMBRE']], hide_index=True, height=700, use_container_width=True)
        ...
        with col_der:
            ...
            with c1:
                with st.container(border=True):
                    ...
                    if 'GÉNERO' in df_fil.columns:
                        d1 = df_fil['GÉNERO'].value_counts().reset_index()
                        fig1 = px.pie(d1, names='GÉNERO', values='count', hole=0.6, color_discrete_sequence=PALETA_AZUL_GRIS)
            ...
            with c2:
                with st.container(border=True):
                    ...
                    if 'CATEGORÍA' in df_fil.columns:
                        d2 = df_fil['CATEGORÍA'].value_counts().reset_index()
                        fig2 = px.pie(d2, names='CATEGORÍA', values='count', hole=0.6, color_discrete_sequence=PALETA_AZUL_GRIS[2:])
            ...
            with c3:
                with st.container(border=True):
                    ...
                    if tipo_col in df_fil.columns:
                        d3 = df_fil[tipo_col].value_counts().reset_index()
                        fig3 = px.pie(d3, names=tipo_col, values='count', hole=0.6, color_discrete_sequence=PALETA_AZUL_GRIS[4:])
            ...
            with c4:
                with st.container(border=True):
                    ...
                    if 'CENTRO DE COSTOS' in df_fil.columns:
                        d4 = df_fil['CENTRO DE COSTOS'].value_counts().reset_index()
                        fig4 = px.pie(d4, names='CENTRO DE COSTOS', values='count', hole=0.6, color_discrete_sequence=PALETA_AZUL_GRIS)
            ...
            with cl2:
                with st.container(border=True):
                    st.markdown("<p style='text-align:center; background-color:#F1F5F9; padding:5px;'><b>Áreas</b></p>", unsafe_allow_html=True)
